...
class MdlDashboard
{
    public static function Dashboard($idFinca)
    {
        ...
        if($resultado > 0)
        {
            while($row = $query->fetch_assoc())
            {
                ...
                if($row['dias'] >= 541 && $row['dias']<=730){$vaquillonas++;}
                if($row['dias'] >= 731){$vacas++;}
            }
        }
    ...
    public static function DashboardInicio()
    {
        $ConexionBD = new ConexionBD();
		$conexion   = $ConexionBD->Abrir();
        $query      = mysqli_query($conexion,"SELECT DATEDIFF(DATE(NOW()),fecha_nacimiento)dias FROM animal WHERE idAnimal=idAnimal");
        $resultado  = mysqli_num_rows($query);
